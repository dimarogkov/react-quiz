...
export const Answer: React.FC<Props> = ({ answer, isCheckBtnSelected, isAnswersSelected, selectAnswer = () => {} }) => {
    return (
        <li
            className={cn(
                'w-full lg:text-[18px] font-medium py-[8px] px-[16px] rounded-[4px] border-2 cursor-pointer mb-[8px] last:mb-0 transition-opacity duration-300 hover:opacity-60',
                {
                    ...
                    'border-slate-200': !isAnswersSelected && !isCheckBtnSelected,
                    'border-slate-950': isAnswersSelected && !isCheckBtnSelected,
                }
            )}
